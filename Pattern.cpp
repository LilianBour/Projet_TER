...
void Pattern::sorting() {
	////Triage et inversement du vecteur en fonction du nombre d'apparition d'un motif pour avoir les motifs qui se répétent le mieux en premier
	sort(counter_pattern_imgs.begin(), counter_pattern_imgs.end());
	reverse(counter_pattern_imgs.begin(), counter_pattern_imgs.end());
	////Suppression du dernier élément car inutile (c'est l'élément avec lequel on initialise le vecteur)
